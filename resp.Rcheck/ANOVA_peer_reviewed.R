#!/usr/bin/R
## Wrapper to optimize exploratory analysis on split-plot experimental design for paper publication. 
# Author: Adri'a Masip

fn_getANOVATable <- function(dataset,resp.var,factors=c('Treatment','Specie'),export=NULL,fact_rel=NULL) {
  ...
  for (i in resp.var) {
    ...
    cat('...',i,'\n')
    list2way[[i]] <- anova.2way(data=dataset,
                                formula=form,
                                nperm = 999)
  }
  #print(warnings())
  ...
  # Create ANOVA table, after Tallent-Halsell et al. 2002
  resp<-c();name<-c();df<-c();SS<-c();MS<-c();Fval<-c();Pperm<-c()
  
  for (l in names(list2way)) {
    tb <- list2way[[l]]$anova.table
    resp <- c(resp,c(l,"","")) # Needs more abstraction
  ...
  for (i in 1:length(Pperm)) {
    if (Pperm[i]<0.01) {x[i]<-"**";next}
    if (Pperm[i]<0.05) {x[i]<-"*"} else {x[i]<-""}
  }
  
  anova_table <- data.frame(resp,name,df,SS,MS,Fval,Pperm,x)
  colnames(anova_table) <- c('Response Variable','Main Factor','df','SS','MS','F','P perm','sig')

 
  if (is.null(export)==FALSE) {
    cat('[?] Writting csv file.\n')
    write.csv(anova_table,paste(as.character(export),'.csv',sep = ""))
  }
  et <- Sys.time()
